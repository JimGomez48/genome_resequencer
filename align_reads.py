...
# def align_read(ref, read, index, k):
#     best_pos = None
#     min_mismatch = 100
...
def map_reads(ref, ref_name, reads_file, k, max_mismatch):
    # NOTE: not using max_mismatch
    ...
    with open(reads_file, 'r') as reads, open(align_file, 'w') as outfile:
        ...
        for line in reads:
            ...
            best_pos1_f, min_mismatch1_f = align_read(ref, read1, index, k)
            best_pos2_f, min_mismatch2_f = align_read(ref, read2, index, k)
            ...
            best_pos1_r, min_mismatch1_r = align_read(ref, read1, index, k)
            best_pos2_r, min_mismatch2_r = align_read(ref, read2, index, k)
...
def main():
    ...
    reads_file = cm.READS_DIR + cm.READS_PRE + genome_name + '.txt'

    ref, ref_name = cm.load_genome(ref_file)
    kmer_length = cm.compute_kmer_length(50, args.max_mismatch)
    align_file = map_reads(ref, ref_name, reads_file, kmer_length, args.max_mismatch)
